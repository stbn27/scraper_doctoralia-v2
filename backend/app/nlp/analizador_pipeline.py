...
def _parsear_argumentos() -> argparse.Namespace:
    ...
    parser.add_argument("--prueba", action="store_true", default=False, help="Activa modo prueba")
    parser.add_argument(
        "--limite",
        type=int,
        default=None,
    ...
    parser.add_argument("--especialidad", type=str, default=None, help="Filtra por especialidad")
    ...
    parser.add_argument("--pausa-entre-lotes", type=float, default=2.0, help="Pausa entre requests LLM reales")
    parser.add_argument(
        "--forzar-reanalisis",
        action="store_true",
        default=False,
...
def _obtener_especialistas(args: argparse.Namespace, limite_llm: int | None) -> list[dict]:
    db = get_mongo_db()
...
def _obtener_opiniones_doctor(doctor_id: int) -> list[dict]:
    db = get_mongo_db()
    cursor = db["opiniones"].find({"doctor_id": doctor_id}).sort("fecha_publicacion", -1)
    return list(cursor)


def _es_clinica(especialista: dict) -> bool:
    scraping_meta = especialista.get("scraping_meta") or {}
    url = scraping_meta.get("url_origen", "") if isinstance(scraping_meta, dict) else str(scraping_meta)
    return "/clinicas/" in url or "/centros-medicos/" in url


def _doc_base(especialista: dict, doctor_id: int, nombre: str, estado: str, datos: dict, resultado_ia: dict, modelo_usado: str) -> dict:
    return {
        "doctor_id": doctor_id,
...
def _resultado(doctor_id: int, nombre: str, estado: str, detalle: str = "", requests_antes: int = 0, requests_despues: int = 0, fatal: bool = False) -> dict:
...
def _procesar_especialista(
    ...
    if _es_clinica(especialista):
        ...
        return _resultado(doctor_id, nombre, "skip", "es clínica", requests_antes, modelo.requests_remotos_realizados)
    ...
    try:
        ...
        if not datos["apto_para_ia"]:
            ...
            guardar_analisis(_doc_base(especialista, doctor_id, nombre, "sin_opiniones", datos, analisis_minimo, "ninguno"))
        ...
        try:
            ...
            detalle = str(exc)
            logger.error("%s | id=%d | detención proveedor/límite: %s", nombre, doctor_id, detalle)
            if _logger_nlp:
                registrar_evento_llm(_logger_nlp, doctor_id, nombre, "error_fatal_modelo", detalle)
            marcar_error(doctor_id, f"Error fatal/límite en modelo: {detalle}")
            return _resultado(doctor_id, nombre, "error", detalle, requests_antes, requests_despues, fatal=isinstance(exc, ErrorProveedorFatal))
        except Exception as exc:
            requests_despues = modelo.requests_remotos_realizados
            logger.error("%s | id=%d | Error en modelo: %s", nombre, doctor_id, str(exc))
            if _logger_nlp:
                registrar_evento_llm(_logger_nlp, doctor_id, nombre, "error_modelo", str(exc))
            marcar_error(doctor_id, f"Error en modelo: {str(exc)}")
            return _resultado(doctor_id, nombre, "error", str(exc), requests_antes, requests_despues)
        ...
        try:
            ...
            if _logger_nlp and respuesta_raw:
                registrar_respuesta_cruda(_logger_nlp, doctor_id, nombre, respuesta_raw, exito=True)
        except ValueError:
            if _logger_nlp and respuesta_raw:
                registrar_respuesta_cruda(_logger_nlp, doctor_id, nombre, respuesta_raw, exito=False)
            logger.warning("%s | id=%d | JSON inválido, reintentando...", nombre, doctor_id)
            if _logger_nlp:
                registrar_evento_llm(_logger_nlp, doctor_id, nombre, "json_invalido", "primer intento no pudo parsearse")
            try:
                if _logger_nlp:
                    registrar_evento_llm(_logger_nlp, doctor_id, nombre, "reintento_inicio")
                respuesta_raw = modelo.analizar(prompt_sistema, prompt_usuario)
                resultado_ia = modelo.parsear_respuesta(respuesta_raw)
                if _logger_nlp and respuesta_raw:
                    registrar_respuesta_cruda(_logger_nlp, doctor_id, nombre, respuesta_raw, exito=True)
                ...
                requests_despues = modelo.requests_remotos_realizados
                if _logger_nlp:
                    registrar_evento_llm(_logger_nlp, doctor_id, nombre, "error_fatal_reintento", str(exc))
                marcar_error(doctor_id, f"Error fatal/límite en reintento: {str(exc)}")
                return _resultado(doctor_id, nombre, "error", str(exc), requests_antes, requests_despues, fatal=isinstance(exc, ErrorProveedorFatal))
            except Exception as exc:
                requests_despues = modelo.requests_remotos_realizados
                if _logger_nlp and respuesta_raw:
                    registrar_respuesta_cruda(_logger_nlp, doctor_id, nombre, respuesta_raw, exito=False)
                if _logger_nlp:
                    registrar_evento_llm(_logger_nlp, doctor_id, nombre, "error_reintento", str(exc))
                marcar_error(doctor_id, f"JSON inválido tras reintento: {str(exc)}")
                return _resultado(doctor_id, nombre, "error", f"JSON inválido: {str(exc)}", requests_antes, requests_despues)
        ...
        _log_success(f"{nombre} | id={doctor_id} | score={score} | confiabilidad={confiabilidad}")
        return _resultado(doctor_id, nombre, "completado", f"score={score}", requests_antes, modelo.requests_remotos_realizados)

    except Exception as exc:
        logger.error("%s | id=%d | Error inesperado: %s", nombre, doctor_id, str(exc))
        if _logger_nlp:
            registrar_evento_llm(_logger_nlp, doctor_id, nombre, "error_inesperado", str(exc))
        marcar_error(doctor_id, f"Error inesperado: {str(exc)}")
        return _resultado(doctor_id, nombre, "error", str(exc), requests_antes, modelo.requests_remotos_realizados)


def _stats_dict(stats: EstadisticasNLP, tiempo_total: float, modelo_nombre: str, modo: str) -> dict:
    data = asdict(stats)
    data.update({
...
def main() -> None:
    ...
    for especialista in especialistas:
        if limite_llm is not None and modelo.requests_remotos_realizados >= limite_llm:
            ...
            estado_persistido = marcar_detencion_nlp(estado_persistido, stats.detenido_por)
            break
        ...
        if resultado.get("estado") in ("completado", "sin_opiniones", "sospecha_fraude"):
        ...
        if resultado.get("fatal_proveedor"):
            ...
            estado_persistido = marcar_detencion_nlp(estado_persistido, stats.detenido_por, fatal_proveedor=True)
            exit_code = 2
            break
    ...
    logger.info(
        ...
        stats.procesados_llm, stats.procesados_localmente, stats.skips, stats.errores,
    )
    logger.info(
